...
translates it, then burns synchronized karaoke subtitles back onto the video.

Pipeline stages
----------------
1.   Audio isolation (audio-separator: BS-RoFormer + Mel-RoFormer ensemble,
     followed by a de-reverb/de-echo pass).
...
4.   ASS karaoke subtitle generation and ffmpeg compilation.
...
logger = logging.getLogger("karaoke_builder")
...
def build_ass_document(phrases: list, translations: list, vid_w: int, vid_h: int) -> str:
    ...
    lines = [
        ...
        f"Style: KaraokeMain,Arial,{main_fs},&H0000FFFF,&H00FFFFFF,&H00000000,"
    ...
    for idx, phrase in enumerate(phrases):
        ...
        for w in words:
            ...
            curr_t = w["end"]
        karaoke = "".join(k_parts).strip()

        translation = translations[idx] if idx < len(translations) else ""
        if translation:
            combined = (
                f"{karaoke}\\N{{\\r\\c&H00A0A0A0&\\fs{trans_fs}}}"
...
def run_pipeline(
    ...
    """Run the end-to-end karaoke pipeline and return the output video path."""
    ...
    base_dir = os.path.abspath(
        os.path.join(tempfile.gettempdir(), f"karaoke_{uuid.uuid4().hex[:8]}")
    )
    ...
    final_vid = os.path.join(
        out_dir, os.path.splitext(file_name)[0] + f"_Karaoke_{target_lang.upper()}.mp4"
    )
