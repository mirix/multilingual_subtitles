...
# Gemma 4 emits its chain-of-thought inside <|channel>thought\n...<channel|> tags.
# processor.parse_response() handles extraction of the final answer in both thinking and
# non-thinking modes (non-E2B/E4B models always emit the tags, even when thinking is
# disabled — just with an empty block).
...
def _load_gemma():
    """Load Gemma 4 26B A4B with 4-bit NF4 quantization for an RTX 3090 (24 GB).

    The full model is 25.2B total parameters; in bfloat16 that is ~50 GB.
    NF4 + double-quant brings the weight footprint to ~13-14 GB, fitting
    comfortably within the 3090's 24 GB budget while retaining more quality
    than plain int8. Compute activations are kept in bfloat16.

    device_map={"": 0} places every layer unconditionally on GPU 0, bypassing
    accelerate's auto-dispatch logic. With device_map="auto", accelerate may
    decide some layers should spill to CPU; bitsandbytes 4-bit rejects any CPU
    placement and raises a ValueError before the model even loads. Explicit
    placement avoids this: if the quantized weights genuinely exceed VRAM you
    get a clean CUDA OOM instead of the misleading dispatch error.

    Returns (model, processor) -- note the second return value is an
    AutoProcessor, not a bare tokenizer, because Gemma 4 uses a unified
    processor that also owns apply_chat_template / parse_response.
    """
...
def execute_llm_task(model, processor, prompt: str, expected_count: int,
    ...
    Gemma 4 emits its reasoning inside <|channel>thought\\n...<channel|> tags.
    processor.parse_response() extracts the final answer from both thinking and
    non-thinking outputs (non-E2B/E4B models always emit the tags, even when thinking
    is disabled — just with an empty block). We therefore always decode with
    skip_special_tokens=False and always call parse_response.

    Recommended Gemma 4 sampling: temperature=1.0, top_p=0.95, top_k=64.
    For non-thinking translation passes we use greedy decoding with a mild repetition
    penalty — more deterministic and cheaper than sampled non-thinking generation.
    """
    messages = [
        {
            "role": "system",
    ...
    try:
        ...
        # Defensive fallback — should not be needed for Gemma 4 but keeps the pipeline alive
        # if a future transformers version changes the signature.
        text = prompt
    ...
    # Gemma 4 26B A4B supports 256k tokens. Warn if we're burning through the budget.
    input_len = inputs["input_ids"].shape[-1]
    if input_len > 200_000:
        logger.warning(
            f"Prompt is {input_len} tokens — approaching Gemma 4's 256k context limit. "
            f"Consider chunking input."
        )
    ...
    if enable_thinking:
        ...
        gen_kwargs = dict(
            ...
            do_sample=False,                           # greedy for non-thinking passes
    ...
    # Decode with skip_special_tokens=False so parse_response can locate the thought tags.
    raw_response = processor.decode(outputs[0][input_len:], skip_special_tokens=False)

    try:
        completion = processor.parse_response(raw_response)
        # parse_response returns a string in current transformers; guard against future dicts.
        if not isinstance(completion, str):
            completion = completion.get("text") or completion.get("answer") or str(completion)
    except Exception:
        # Fallback: strip the Gemma 4 thought block manually if parse_response is unavailable.
        logger.warning(
            "processor.parse_response() failed; falling back to regex-based thought stripping."
        )
        completion = re.sub(
            r'<\|channel>thought\n.*?<channel\|>', '', raw_response, flags=re.DOTALL
        ).strip()

    return _parse_numbered_lines(completion, expected_count)


def execute_llm_task_with_recovery(model, processor, prompt_builder, expected_count: int,
    ...
    result = execute_llm_task(model, processor, primary_prompt, expected_count,
    ...
    recovered = execute_llm_task(model, processor, recovery_prompt, len(omitted),
                                 enable_thinking=enable_thinking)
...
def run_pipeline(video_path: str, target_lang: str, source_override: str = "auto",
    ...
    try:
        ...
        logger.info(f"Phase 2.5: Loading Corrector LLM ({GEMMA_MODEL_ID})...")
        model_corr, processor_corr = _load_gemma()
        flush_vram("after corrector load")

        # Gemma 4 thinking mode — the reasoning chain helps catch homophones and contextual
        # inconsistencies that a non-thinking pass would miss. The thought block is stripped
        # from the output by processor.parse_response() (see execute_llm_task).
        corrected_texts = execute_llm_task_with_recovery(
            model_corr, processor_corr,
        ...
        _release_model(model_corr, processor_corr)
        ...
        if needs_translation:
            logger.info(f"Phase 3: Loading Translator LLM ({GEMMA_MODEL_ID})...")
            model_trans, processor_trans = _load_gemma()
            flush_vram("after translator load")

            translations = execute_llm_task_with_recovery(
                model_trans, processor_trans,
            ...
            _release_model(model_trans, processor_trans)
